  ...
  return (
    <div className={`flex ${isOwn ? 'justify-end' : 'justify-start'} mb-3 group`}>
      <div className={`flex gap-2.5 max-w-[80%] ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
        {!isOwn && (
          <div className="w-8 h-8 bg-muted text-text-secondary flex items-center justify-center text-xs font-bold flex-shrink-0 mt-6">
            ...
            {isOwn && (
              <div className="relative flex-shrink-0 mt-[7px]">
                <button onClick={() => setMenuOpen(!menuOpen)}
                  className="opacity-0 group-hover:opacity-100 w-6 h-6 flex items-center justify-center text-text-secondary hover:bg-muted">
  ...
  return (
    <>
      ...
      <div ref={menuRef} className={`absolute z-20 bg-panel border border-border py-1 min-w-[140px] ${
        ...
        {onReply && (
          <button onClick={onReply} className="w-full text-left px-3.5 py-2 text-xs text-text-secondary hover:bg-muted flex items-center gap-2">
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" /></svg>
            Reply
          </button>
        )}
        {onForward && (
          <button onClick={onForward} className="w-full text-left px-3.5 py-2 text-xs text-text-secondary hover:bg-muted flex items-center gap-2">
        ...
        {onDelete && (
          <>
            ...
            <button onClick={onDelete} className="w-full text-left px-3.5 py-2 text-xs text-accent-terracotta hover:bg-muted flex items-center gap-2">
