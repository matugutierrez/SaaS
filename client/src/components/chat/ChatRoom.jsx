...
export default function ChatRoom({ roomId, roomName }) {
  ...
  return (
    <div className="flex-1 flex flex-col bg-white">
      <div className="px-5 py-3.5 border-b border-gray-50 bg-white flex items-center gap-3">
        <div className="w-2.5 h-2.5 rounded-full bg-green-500" />
        <h3 className="font-semibold text-sm text-gray-800">#{roomName}</h3>
      </div>
        ...
        {loading ? (
          <div className="space-y-4">
            {[1,2,3].map(i => <div key={i} className={`flex ${i % 2 === 0 ? 'justify-end' : 'justify-start'}`}>
              <div className={`h-12 ${i % 2 === 0 ? 'w-48' : 'w-36'} bg-gray-200 rounded-2xl animate-pulse`} />
            </div>)}
          </div>
        ) : (
          <>
            {page < totalPages && (
              <button onClick={loadMore}
                className="text-xs text-primary-600 font-medium hover:underline w-full text-center py-3 bg-white rounded-xl shadow-sm mb-4 hover:bg-gray-50 transition">
                Load older messages
              </button>
            )}
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <p className="text-gray-300 text-xs mt-1">Be the first to say something</p>
              </div>
            ) : (
